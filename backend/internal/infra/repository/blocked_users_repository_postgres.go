...
type BlockedUsersRepositoryPostgres struct {
	db *sqlx.DB
}

func NewBlockedUsersRepositoryPostgres(db *sqlx.DB) *NewBlockedUsersRepositoryPostgres {
	return &NewBlockedUsersRepositoryPostgres{db: db}
}

func (r *BlockedUsersRepositoryPostgres) CreateBlockedUser(blockedUser *entity.) (*entity.Medicine, error) {
	err := r.db.QueryRow("INSERT INTO BlockedUsers (user_id, name, stripe) VALUES ($1, $2, $3) RETURNING id, created_at, updated_at", medice.Batch, medice.Name, medice.Stripe).Scan(&medice.ID, &medice.CreatedAt, &medice.UpdatedAt)
